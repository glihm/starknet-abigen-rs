...
macro_rules! implement_trait_for_signed {
    ($type:ty) => {
        impl CairoType for $type {
            ...
            fn deserialize(felts: &[FieldElement], offset: usize) -> Result<Self::RustType> {
                let temp: u16 = felts[offset].try_into().unwrap();
                Ok(temp as $type)
            }
        }
    };
...
mod tests {
    ...
    fn test_serialize_i8() {
        ...
        assert_eq!(felts.len(), 1);
        assert_eq!(felts[0], FieldElement::from(123 as u8));
    }

    #[test]
    fn test_deserialize_i8() {
        let felts = vec![FieldElement::from(123 as u8), FieldElement::from(123 as u8)];
        assert_eq!(i8::deserialize(&felts, 0).unwrap(), 123);
        assert_eq!(i8::deserialize(&felts, 1).unwrap(), 123);
    }
    ...
    fn test_deserialize_i16() {
        let felts = vec![FieldElement::from(123 as u8), FieldElement::from(123 as u8)];
        assert_eq!(i16::deserialize(&felts, 0).unwrap(), 123);
        assert_eq!(i16::deserialize(&felts, 1).unwrap(), 123);
    }
    ...
    fn test_deserialize_i32() {
        let felts = vec![FieldElement::from(123 as u8), FieldElement::from(123 as u8)];
        assert_eq!(i32::deserialize(&felts, 0).unwrap(), 123);
        assert_eq!(i32::deserialize(&felts, 1).unwrap(), 123);
    }
    ...
    fn test_deserialize_i64() {
        let felts = vec![FieldElement::from(123 as u8), FieldElement::from(123 as u8)];
        assert_eq!(i64::deserialize(&felts, 0).unwrap(), 123);
        assert_eq!(i64::deserialize(&felts, 1).unwrap(), 123);
    }

    #[test]
    fn test_serialize_i128() {
        let v = 123_i128;
        let felts = i128::serialize(&v);
        assert_eq!(felts.len(), 1);
        assert_eq!(felts[0], FieldElement::from(123 as u8));
    }
    ...
    fn test_deserialize_i128() {
        let felts = vec![FieldElement::from(123 as u8), FieldElement::from(123 as u8)];
        assert_eq!(i128::deserialize(&felts, 0).unwrap(), 123);
        assert_eq!(i128::deserialize(&felts, 1).unwrap(), 123);
    }
